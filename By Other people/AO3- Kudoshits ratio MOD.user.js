...
var [lvl1, lvl2] = [4, 7];
var [ratio_red, ratio_yellow, ratio_green] = [`#A00000`, `#9F8600`, `#289200`];
...
/* INCLUE jQuery LOCALLY AS A FUNCTION */
// -------------------------------------------------
/*!
		...
		if (countable) {
			...
			$('dl.stats').each(function () {
				...
				if (kudos_value.length && hits_value.length && hits_value.text() !== '0') {
					...
					var ratio_value = $('<dd class="kudoshits"></dd>').text(percents_print + '%').css('color', '#2a2a2a');
					hits_value.after('\n', ratio_label, '\n', ratio_value);
