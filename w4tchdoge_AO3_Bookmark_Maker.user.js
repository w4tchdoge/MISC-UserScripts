...
/*
	...
	Explanation on how to add your own customisations to AutoTag is provided in the comment above the AutoTag function,
	which can be found right before the configuration area mentioned above.
	An explanation with an example is available in the following GreasyFork comment:
	https://greasyfork.org/en/scripts/467885/discussions/323356#comment-638498
...
(async function () {
	...
	let initial_settings_dict = {
		...
		includeReadDate: false,
		showAutoTagButton: true,
		partOfSeriesAutoTag: false,
		partOfSeriesAutoTagCustom: false,
		partOfSeriesAutoTagCustomValue: `N/A`,
		seriesAutoTag: false,
		seriesAutoTagCustom: false,
		seriesAutoTagCustomValue: `N/A`,
		bottomSummaryPage: true,
		topSummaryPage: false,
		simpleWorkSummary: false,
		FWS_asBlockquote: true,
		alwaysInjectBookmark: true,
		AutoTag_wordcount_type: 0,
		splitSelect: 1,
	};
...
includeWordCount               : If true, include word count tag(s) for the work/series as user tags when running the Auto Tag function


includeReadDate                : If true, include tags for the month and year the work/series was lat read as user tags when running the Auto Tag function (as suggested by `Mimi123123` in https://greasyfork.org/en/scripts/467885/discussions/323356)


showAutoTagButton              : If true, shows the "Auto Tag" button when bookmarking a work


partOfSeriesAutoTag            : If true, if the work is a part of a series adds the string "Part of Series" to the user tags when running the Auto Tag function


partOfSeriesAutoTagCustom      : If true, use a custom user-defined string for partOfSeriesAutoTag


partOfSeriesAutoTagCustomValue : Variable that stores the custom user-defined string to be user in partOfSeriesAutoTag if partOfSeriesAutoTagCustom is true


seriesAutoTag                  : If true, if the bookmark is for a series adds the string "Series" to the user tags when running the Auto Tag function


seriesAutoTagCustom            : If true, use a custom user-defined string for seriesAutoTag


seriesAutoTagCustomValue       : Variable that stores the custom user-defined string to be user in seriesAutoTag if seriesAutoTagCustom is true
...
AutoTag_wordcount_type         : Determines how the Word Count section of AutoTag function works.
0 indicates it is using the original behaviour of AutoTag's Word Count as suggested by `oliver t` in https://greasyfork.org/en/scripts/467885/discussions/198028 that was present when AutoTag was first added to the userscript
...
Another way to explain it is that the script works by taking the current contents of your bookmark and splitting it into two pieces along a user defined "divider", thus creating an array. Depending on the way you want the bookmark to be structured, the part of the bookmark that you want to keep could be before the divider or after the divider. splitSelect lets you tell the script which half of the array contains the bit of the bookmark you want to keep. If it's the first half splitSelect is 0, if it's the last half splitSelect is 1.
	...
	*/
	...
	let
		...
		showAutoTagButton,
		partOfSeriesAutoTag,
		partOfSeriesAutoTagCustom,
		partOfSeriesAutoTagCustomValue,
		seriesAutoTag,
		seriesAutoTagCustom,
		seriesAutoTagCustomValue,
		bottomSummaryPage,
		topSummaryPage,
		simpleWorkSummary,
		FWS_asBlockquote,
		alwaysInjectBookmark,
		AutoTag_wordcount_type,
		splitSelect,
		workInfo;
...
'w4BM_includeFandom' IS SET in the localStorage`
		...
		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_includeWordCount`))) {
			case false:
				console.log(`
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_includeWordCount' is not set in the localStorage
Now setting it to '${initial_settings_dict.includeWordCount}'`
				);

				includeWordCount = initial_settings_dict.includeWordCount;
				localStorage.setItem(`w4BM_includeWordCount`, initial_settings_dict.includeWordCount);
...
'w4BM_includeWordCount' IS SET in the localStorage`
			...
			default:
				console.log(`Error in retrieving localStorage variable w4BM_includeWordCount`);
				break;
		}

		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_includeReadDate`))) {
			case false:
				console.log(`
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_includeReadDate' is not set in the localStorage
Now setting it to '${initial_settings_dict.includeReadDate}'`
				);

				includeReadDate = initial_settings_dict.includeReadDate;
				localStorage.setItem(`w4BM_includeReadDate`, initial_settings_dict.includeReadDate);
...
'w4BM_includeReadDate' IS SET in the localStorage`
			...
			default:
				console.log(`Error in retrieving localStorage variable w4BM_includeReadDate`);
				break;
		}
...
'w4BM_showAutoTagButton' IS SET in the localStorage`
		...
		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_partOfSeriesAutoTag`))) {
			case false:
				const old_localStorage_var = localStorage.getItem(`w4BM_seriesAutoTag`);

				if (Boolean(old_localStorage_var)) {

					const log_str = `
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_partOfSeriesAutoTag' is not set in the localStorage
Now setting it to '${old_localStorage_var}'`;
					console.log(log_str);

					partOfSeriesAutoTag = old_localStorage_var;
					localStorage.setItem(`w4BM_partOfSeriesAutoTag`, old_localStorage_var);

					if (old_localStorage_var == true || old_localStorage_var == `true`) {
						localStorage.setItem(`w4BM_partOfSeriesAutoTagCustom`, true);
						localStorage.setItem(`w4BM_partOfSeriesAutoTagCustomValue`, `Series`);
					}

					localStorage.removeItem(`w4BM_seriesAutoTag`);
...
--------------------
'w4BM_partOfSeriesAutoTag' is not set in the localStorage
Now setting it to '${initial_settings_dict.partOfSeriesAutoTag}'`;
					console.log(log_str);

					partOfSeriesAutoTag = initial_settings_dict.partOfSeriesAutoTag;
					localStorage.setItem(`w4BM_partOfSeriesAutoTag`, initial_settings_dict.partOfSeriesAutoTag);
...
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_partOfSeriesAutoTag' IS SET in the localStorage`
				);

				partOfSeriesAutoTag = stringToBoolean(localStorage.getItem(`w4BM_partOfSeriesAutoTag`));

				break;

			default:
				console.log(`Error in retrieving localStorage variable w4BM_partOfSeriesAutoTag`);
				break;
		}

		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_partOfSeriesAutoTagCustom`))) {
			case false:
				console.log(`
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_partOfSeriesAutoTagCustom' is not set in the localStorage
Now setting it to '${initial_settings_dict.partOfSeriesAutoTagCustom}'`
				);

				partOfSeriesAutoTagCustom = initial_settings_dict.partOfSeriesAutoTagCustom;
				localStorage.setItem(`w4BM_partOfSeriesAutoTagCustom`, initial_settings_dict.partOfSeriesAutoTagCustom);
...
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_partOfSeriesAutoTagCustom' IS SET in the localStorage`
				);

				partOfSeriesAutoTagCustom = stringToBoolean(localStorage.getItem(`w4BM_partOfSeriesAutoTagCustom`));

				break;

			default:
				console.log(`Error in retrieving localStorage variable w4BM_partOfSeriesAutoTagCustom`);
				break;
		}

		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_partOfSeriesAutoTagCustomValue`))) {
			case false:
				console.log(`
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_partOfSeriesAutoTagCustomValue' is not set in the localStorage
Now setting it to '${initial_settings_dict.partOfSeriesAutoTagCustomValue.replace(/\n/gi, '').replace(/\t/gi, '').replace(/\r/gi, '')}'`
				);

				partOfSeriesAutoTagCustomValue = initial_settings_dict.partOfSeriesAutoTagCustomValue;
				localStorage.setItem(`w4BM_partOfSeriesAutoTagCustomValue`, initial_settings_dict.partOfSeriesAutoTagCustomValue);
...
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_partOfSeriesAutoTagCustomValue' IS SET in the localStorage`
				);

				partOfSeriesAutoTagCustomValue = localStorage.getItem(`w4BM_partOfSeriesAutoTagCustomValue`);

				break;

			default:
				console.log(`Error in retrieving localStorage variable w4BM_partOfSeriesAutoTagCustomValue`);
				break;
		}
...
'w4BM_seriesAutoTag' IS SET in the localStorage`
		...
		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_seriesAutoTagCustom`))) {
			case false:
				console.log(`
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_seriesAutoTagCustom' is not set in the localStorage
Now setting it to '${initial_settings_dict.seriesAutoTagCustom}'`
				);

				seriesAutoTagCustom = initial_settings_dict.seriesAutoTagCustom;
				localStorage.setItem(`w4BM_seriesAutoTagCustom`, initial_settings_dict.seriesAutoTagCustom);
...
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_seriesAutoTagCustom' IS SET in the localStorage`
				);

				seriesAutoTagCustom = stringToBoolean(localStorage.getItem(`w4BM_seriesAutoTagCustom`));

				break;

			default:
				console.log(`Error in retrieving localStorage variable w4BM_seriesAutoTagCustom`);
				break;
		}

		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_seriesAutoTagCustomValue`))) {
			case false:
				console.log(`
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_seriesAutoTagCustomValue' is not set in the localStorage
Now setting it to '${initial_settings_dict.seriesAutoTagCustomValue.replace(/\n/gi, '').replace(/\t/gi, '').replace(/\r/gi, '')}'`
				);

				seriesAutoTagCustomValue = initial_settings_dict.seriesAutoTagCustomValue;
				localStorage.setItem(`w4BM_seriesAutoTagCustomValue`, initial_settings_dict.seriesAutoTagCustomValue);
...
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_seriesAutoTagCustomValue' IS SET in the localStorage`
				);

				seriesAutoTagCustomValue = localStorage.getItem(`w4BM_seriesAutoTagCustomValue`);

				break;

			default:
				console.log(`Error in retrieving localStorage variable w4BM_seriesAutoTagCustomValue`);
				break;
		}
...
'w4BM_alwaysInjectBookmark' IS SET in the localStorage`
		...
		// doing the same thing as the first if else on line 233
		switch (Boolean(localStorage.getItem(`w4BM_AutoTag_wordcount_type`))) {
			case false:
				const old_localStorage_var = localStorage.getItem(`w4BM_AutoTag_type`);
...
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_AutoTag_wordcount_type' is not set in the localStorage
Now setting it to '${old_localStorage_var}'`;
					console.log(log_str);

					AutoTag_wordcount_type = old_localStorage_var;
					localStorage.setItem(`w4BM_AutoTag_wordcount_type`, old_localStorage_var);

					localStorage.removeItem(`w4BM_AutoTag_type`);
...
--------------------
'w4BM_AutoTag_wordcount_type' is not set in the localStorage
Now setting it to '${initial_settings_dict.AutoTag_wordcount_type}'`;
					console.log(log_str);

					AutoTag_wordcount_type = initial_settings_dict.AutoTag_wordcount_type;
					localStorage.setItem(`w4BM_AutoTag_wordcount_type`, initial_settings_dict.AutoTag_wordcount_type);
...
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
'w4BM_AutoTag_wordcount_type' IS SET in the localStorage`
				);

				AutoTag_wordcount_type = parseInt(localStorage.getItem(`w4BM_AutoTag_wordcount_type`));

				break;

			default:
				console.log(`Error in retrieving localStorage variable w4BM_AutoTag_wordcount_type`);
				break;
		}
...
includeReadDate                : ${includeReadDate}
showAutoTagButton              : ${showAutoTagButton}
partOfSeriesAutoTag            : ${partOfSeriesAutoTag}
partOfSeriesAutoTagCustom      : ${partOfSeriesAutoTagCustom}
partOfSeriesAutoTagCustomValue : ${partOfSeriesAutoTagCustomValue.replace(/\n/gi, `\\n`).replace(/\t/gi, `\\t`).replace(/\r/gi, `\\r`)}
seriesAutoTag                  : ${seriesAutoTag}
seriesAutoTagCustom            : ${seriesAutoTagCustom}
seriesAutoTagCustomValue       : ${seriesAutoTagCustomValue.replace(/\n/gi, `\\n`).replace(/\t/gi, `\\t`).replace(/\r/gi, `\\r`)}
...
AutoTag_wordcount_type         : ${AutoTag_wordcount_type}
splitSelect                    : ${splitSelect}`;
	...
	function addPrefDropdown() {
		...
		function createAutoTagCustomValueInputArea(localstorage_key_name, description) {
			// create the general area where the input will go
			const input_area = Object.assign(document.createElement(`li`), {
				className: `${localstorage_key_name}_input_area`,
				id: `${localstorage_key_name}_input_area`,
				style: `padding-top: .75em;`,
				innerHTML: `<p style='padding: .75em .5em .5em;'>${description}<br /></p>`
			});

			// create the text input box for the input value
			const input_box = (function () {
				let div_in_box = Object.assign(document.createElement(`input`), {
					type: `text`,
					id: `${localstorage_key_name}_input_box`,
					name: `${localstorage_key_name}_input_box`,
					style: `width: 20.4em; margin-left: 0.2em;`
				});

				div_in_box.setAttribute(`value`, localStorage.getItem(localstorage_key_name).replace(/\n/gi, '').replace(/\t/gi, '').replace(/\r/gi, '') || 'N/A');
			...
			// create the input submit button
			const input_btn = Object.assign(document.createElement(`button`), {
				id: `${localstorage_key_name}_input_btn`,
				style: `margin-left: 0.3em;`,
				innerHTML: `Enter`
			});

			// append the input submit button
			input_box.after(input_btn);

			// make the input submit button actually do what it's supposed to
			input_btn.addEventListener(`click`, function () {
				const input_value = unescapeSlashes(document.querySelector(`#${localstorage_key_name}_input_box`).value).replace(/\n/gi, '').replace(/\t/gi, '').replace(/\r/gi, '');
				localStorage.setItem(localstorage_key_name, input_value);
				console.log(`
w4tchdoge's AO3 Bookmark Maker UserScript – Log
--------------------
New value set for the '${localstorage_key_name}' variable in localStorage.
...
${input_value}`
		...
		// create button - AutoTag: Add tag for when work is part of series - yes
		const w4BM_partOfSeriesAutoTag_yes = Object.assign(document.createElement(`li`), {
			className: `w4BM_partOfSeriesAutoTag_yes`,
			id: `w4BM_partOfSeriesAutoTag_yes`,
			innerHTML: `<a>Autotag, Work is Part of Series: YES</a>`
		});
		w4BM_partOfSeriesAutoTag_yes.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_partOfSeriesAutoTag`, false);
			w4BM_partOfSeriesAutoTag_yes.replaceWith(w4BM_partOfSeriesAutoTag_no);
		});

		// create button - AutoTag: Add tag for when work is part of series - no
		const w4BM_partOfSeriesAutoTag_no = Object.assign(document.createElement(`li`), {
			className: `w4BM_partOfSeriesAutoTag_no`,
			id: `w4BM_partOfSeriesAutoTag_no`,
			innerHTML: `<a>Autotag, Work is Part of Series: NO</a>`
		});
		w4BM_partOfSeriesAutoTag_no.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_partOfSeriesAutoTag`, true);
			w4BM_partOfSeriesAutoTag_no.replaceWith(w4BM_partOfSeriesAutoTag_yes);
		});

		// create button - AutoTag: Use custom tag for when work is part of series - yes
		const w4BM_partOfSeriesAutoTagCustom_yes = Object.assign(document.createElement(`li`), {
			className: `w4BM_partOfSeriesAutoTagCustom_yes`,
			id: `w4BM_partOfSeriesAutoTagCustom_yes`,
			innerHTML: `<a>Autotag, Use Custom Tag for Work is Part of Series: YES</a>`
		});
		w4BM_partOfSeriesAutoTagCustom_yes.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_partOfSeriesAutoTagCustom`, false);
			w4BM_partOfSeriesAutoTagCustom_yes.replaceWith(w4BM_partOfSeriesAutoTagCustom_no);
		});

		// create button - AutoTag: Use custom tag for when work is part of series - no
		const w4BM_partOfSeriesAutoTagCustom_no = Object.assign(document.createElement(`li`), {
			className: `w4BM_partOfSeriesAutoTagCustom_no`,
			id: `w4BM_partOfSeriesAutoTagCustom_no`,
			innerHTML: `<a>Autotag, Use Custom Tag for Work is Part of Series: NO</a>`
		});
		w4BM_partOfSeriesAutoTagCustom_no.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_partOfSeriesAutoTagCustom`, true);
			w4BM_partOfSeriesAutoTagCustom_no.replaceWith(w4BM_partOfSeriesAutoTagCustom_yes);
		});

		// create the input area for partOfSeriesAutoTagCustomValue
		const w4BM_partOfSeriesAutoTagCustomValue_input_area = createAutoTagCustomValueInputArea('w4BM_partOfSeriesAutoTagCustomValue', 'Custom tag text for "Autotag: Work is Part of Series":');
		...
		const w4BM_seriesAutoTag_yes = Object.assign(document.createElement(`li`), {
			...
			innerHTML: `<a>Autotag, Bookmark is a Series: YES</a>`
		});
		...
		// create button - AutoTag: Add tag for when bookmark is for a series - no
		const w4BM_seriesAutoTag_no = Object.assign(document.createElement(`li`), {
			...
			innerHTML: `<a>Autotag, Bookmark is a Series: NO</a>`
		});
		...
		// create button - AutoTag: Use custom tag for when bookmark is for a series - yes
		const w4BM_seriesAutoTagCustom_yes = Object.assign(document.createElement(`li`), {
			className: `w4BM_seriesAutoTagCustom_yes`,
			id: `w4BM_seriesAutoTagCustom_yes`,
			innerHTML: `<a>Autotag, Use Custom Tag for Bookmark is Series: YES</a>`
		});
		w4BM_seriesAutoTagCustom_yes.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_seriesAutoTagCustom`, false);
			w4BM_seriesAutoTagCustom_yes.replaceWith(w4BM_seriesAutoTagCustom_no);
		});

		// create button - AutoTag: Use custom tag for when bookmark is for a series - no
		const w4BM_seriesAutoTagCustom_no = Object.assign(document.createElement(`li`), {
			className: `w4BM_seriesAutoTagCustom_no`,
			id: `w4BM_seriesAutoTagCustom_no`,
			innerHTML: `<a>Autotag, Use Custom Tag for Bookmark is Series: NO</a>`
		});
		w4BM_seriesAutoTagCustom_no.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_seriesAutoTagCustom`, true);
			w4BM_seriesAutoTagCustom_no.replaceWith(w4BM_seriesAutoTagCustom_yes);
		});

		// create the input area for seriesAutoTagCustomValue
		const w4BM_seriesAutoTagCustomValue_input_area = createAutoTagCustomValueInputArea('w4BM_seriesAutoTagCustomValue', 'Custom tag text for "Autotag, Bookmark is a Series":');
		...
		// create button - include word count in AutoTag - yes
		const w4BM_includeWordCountButton_yes = Object.assign(document.createElement(`li`), {
			className: `w4BM_includeWordCountButton_yes`,
			id: `w4BM_includeWordCountButton_yes`,
			innerHTML: `<a>Include Word Count in AutoTag: YES</a>`
		});
		w4BM_includeWordCountButton_yes.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_includeWordCount`, false);
			w4BM_includeWordCountButton_yes.replaceWith(w4BM_includeWordCountButton_no);
		});

		// create button - include word count in AutoTag - no
		const w4BM_includeWordCountButton_no = Object.assign(document.createElement(`li`), {
			className: `w4BM_includeWordCountButton_no`,
			id: `w4BM_includeWordCountButton_no`,
			innerHTML: `<a>Include Word Count in AutoTag: NO</a>`
		});
		w4BM_includeWordCountButton_no.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_includeWordCount`, true);
			w4BM_includeWordCountButton_no.replaceWith(w4BM_includeWordCountButton_yes);
		});

		// create button - AutoTag type 0
		const w4BM_AutoTagWCType_0 = Object.assign(document.createElement(`li`), {
			className: `w4BM_AutoTagType_0`,
			id: `w4BM_AutoTagType_0`,
			innerHTML: `<a>AutoTag Type: Original</a>`
		});
		w4BM_AutoTagWCType_0.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_AutoTag_wordcount_type`, 1);
			w4BM_AutoTagWCType_0.replaceWith(w4BM_AutoTagWCType_1);
		});

		// create button - AutoTag type 1
		const w4BM_AutoTagWCType_1 = Object.assign(document.createElement(`li`), {
			className: `w4BM_AutoTagType_1`,
			id: `w4BM_AutoTagType_1`,
			innerHTML: `<a>AutoTag Type: Canon AO3 Wordcount tags</a>`
		});
		w4BM_AutoTagWCType_1.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_AutoTag_wordcount_type`, 0);
			w4BM_AutoTagWCType_1.replaceWith(w4BM_AutoTagWCType_0);
		});

		// create button - include read date in AutoTag - yes
		const w4BM_includeReadDateButton_yes = Object.assign(document.createElement(`li`), {
			className: `w4BM_includeReadDateButton_yes`,
			id: `w4BM_includeReadDateButton_yes`,
			innerHTML: `<a>Include Read Date in AutoTag: YES</a>`
		});
		w4BM_includeReadDateButton_yes.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_includeReadDate`, false);
			w4BM_includeReadDateButton_yes.replaceWith(w4BM_includeReadDateButton_no);
		});

		// create button - include read date in AutoTag - no
		const w4BM_includeReadDateButton_no = Object.assign(document.createElement(`li`), {
			className: `w4BM_includeReadDateButton_no`,
			id: `w4BM_includeReadDateButton_no`,
			innerHTML: `<a>Include Read Date in AutoTag: NO</a>`
		});
		w4BM_includeReadDateButton_no.addEventListener(`click`, function (event) {
			localStorage.setItem(`w4BM_includeReadDate`, true);
			w4BM_includeReadDateButton_no.replaceWith(w4BM_includeReadDateButton_yes);
		});
		...
		if (typeof (Storage) !== `undefined`) {
			...
			// showing partOfSeriesAutoTag button
			if (partOfSeriesAutoTag == true || partOfSeriesAutoTag == `true`) {
				w4BM_dropMenu.append(w4BM_partOfSeriesAutoTag_yes);
			}
			else {
				w4BM_dropMenu.append(w4BM_partOfSeriesAutoTag_no);
			}

			// showing partOfSeriesAutoTagCustom button
			if (partOfSeriesAutoTagCustom == true || partOfSeriesAutoTagCustom == `true`) {
				w4BM_dropMenu.append(w4BM_partOfSeriesAutoTagCustom_yes);
			}
			else {
				w4BM_dropMenu.append(w4BM_partOfSeriesAutoTagCustom_no);
			}

			// showing partOfSeriesAutoTagCustomValue input area
			w4BM_dropMenu.append(w4BM_partOfSeriesAutoTagCustomValue_input_area);
			...
			// showing seriesAutoTag button
			if (seriesAutoTagCustom == true || seriesAutoTagCustom == `true`) {
				w4BM_dropMenu.append(w4BM_seriesAutoTagCustom_yes);
			}
			else {
				w4BM_dropMenu.append(w4BM_seriesAutoTagCustom_no);
			}

			// showing seriesAutoTagCustomValue input area
			w4BM_dropMenu.append(w4BM_seriesAutoTagCustomValue_input_area);
			...
			// showing includeWordCount button
			if (includeWordCount == true || includeWordCount == `true`) {
				w4BM_dropMenu.append(w4BM_includeWordCountButton_yes);
			} else {
				w4BM_dropMenu.append(w4BM_includeWordCountButton_no);
			}

			// choosing AutoTag type button
			if (AutoTag_wordcount_type === 0 || AutoTag_wordcount_type == 0 || AutoTag_wordcount_type == `0`) {
				w4BM_dropMenu.append(w4BM_AutoTagWCType_0);
			} else {
				w4BM_dropMenu.append(w4BM_AutoTagWCType_1);
			}

			// showing includeReadDate button
			if (includeReadDate == true || includeReadDate == `true`) {
				w4BM_dropMenu.append(w4BM_includeReadDateButton_yes);
			} else {
				w4BM_dropMenu.append(w4BM_includeReadDateButton_no);
			}
...
All conditions met for "Summary Page" button in the top nav bar?: ${TSP_conditional}`
	...
	if (script_execute_conditional) {
		...
		if (Boolean(document.querySelector(`#bookmark-form`)) && autoAutoTag) {
			Array.from(document.querySelectorAll(`a.bookmark_form_placement_open[href^="#bookmark"]`))
				.map(bookmark_form_btn => {
					bookmark_form_btn.addEventListener(`click`, AutoTag);
				});
		}
		...
		const IS_SERIES_PART = (() => {
			if (IS_SERIES) {
				...
				return Boolean(main.querySelector(`dl.work.meta.group > dd.series`));
			}
		})();
...
${work_series_info.join(`\n`)}
		...
		/*
		The Auto Tag function

		Feel free to edit the AutoTag function to add anything you'd like. You can add any custom string to the user tag input box by pushing the string
		to the array of tag inputs using tag_inputs.push('YOUR STRING HERE')
		This has to be done before the user tags input box is filled with the contents of tag_inputs array, which is done with tag_input_box.value = tag_inputs.join(`, `);
		So long as you push to the tag_inputs array before that, your custom string will be added to the user tags input area.

		An example of how to do this is available at https://greasyfork.org/en/scripts/467885/discussions/323356#comment-638498
	*/
		...
		async function AutoTag() {
			...
			// Add the string "Part of Series" or the user defined custom value to tag_inputs if work is a series
			if (IS_SERIES_PART && partOfSeriesAutoTag) {
				if (partOfSeriesAutoTagCustom) {
					tag_inputs.push(partOfSeriesAutoTagCustomValue.toString());
				} else {
					tag_inputs.push(`Part of Series`);
				}
			}

			// Add the string "Series" or the user defined custom value to tag_inputs if bookmark is for a series
			if (IS_SERIES && seriesAutoTag) {
				if (seriesAutoTagCustom) {
					tag_inputs.push(seriesAutoTagCustomValue.toString());
				} else {
					tag_inputs.push(`Series`);
				}
			}
			...
			if (includeWordCount) {
				...
				if (inRange(AutoTag_wordcount_type, 0, 1) && AutoTag_wordcount_type == 0) {
					let word_count_tag = ``;
				...
				if (inRange(AutoTag_wordcount_type, 0, 1) && AutoTag_wordcount_type == 1) {
				...
				if (!inRange(AutoTag_wordcount_type, 0, 1)) { console.log(`AutoTag_wordcount_type is not between 0 and 1. Please contact me (the script author) on GreasyFork for troubleshooting`); }
			}

			const MONTH_DICT = Object.freeze([
				`January`,
				`February`,
				`March`,
				`April`,
			...
			if (includeReadDate) {
				...
				const year_str = `read in ${date_obj.getFullYear()}`;
				const month_str = `${MONTH_DICT[date_obj.getMonth()]}${date_obj.getFullYear().toString().slice(-2)}`.toLowerCase();
				tag_inputs.push(year_str, month_str);
			}
...
\t${summary}</details>
		...
		/*
		NOTE:
		Due to how AO3 post-processes the text you enter into the Bookmarks textbox, the script will add a single <br /> tag on a line with nothing else which will make the start of your bookmark look like the following:

		new note line 1<br />
		new note line 2<br />
		<br />
		<details><summary>Work Details</summary>
		{THE_REST_OF_THE_BOOKMARK}

		It's important that the single <br /> tag not be deleted because it forces AO3's bookmark text post-processing to add it's own <br /> tags in
		a way that makes it possible for the script to differentiate the text which is notes you've added and the text which the script has generated
		*/
...
${date_string}</details>`; */
		...
		It's important that the single <br />hile tag not be deleted because it forces AO3's bookmark text post-processing to add it's own <br /> tags
		in a way that makes it possible for the script to differentiate the text which is notes you've added and the text which the script has generated
		*/
